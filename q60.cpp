...
vector<string> getN(string s,tr1::unordered_set<string> &dict)
{
	const char * p = s.c_str();
	vector<string> v;
	for(int i=0;i<=s.length();i++){
		char origin_char = s[i];
		for(char c='a';c<='z';c++)
		{
			if(c!=origin_char){
				s[i] = c;
...
int main()
{
	...
	start = clock();
	cout << "steps " << ladderLength(start_str,end_str,dict) <<endl;
